...
SELECT 
  ...
  site_nominal_mapped,
  microcluster,
  CONCAT('From ndb ', FORMAT_DATE('%b %y', DATE(CAST(vdt_id AS STRING)))) AS remarks,
  system,
  gladiator_branch,
  new_region,
  h3i_technical_branch,
...
drop table if exists `data-bi-prd-935c.bi_stg.site_ref_dim_prep_1`;
...
drop table if exists `data-bi-prd-935c.bi_stg.site_ref_dim_prep_2`;
...
from `data-bi-prd-935c.bi_stg.site_ref_dim_prep_2`;


drop table if exists `data-bi-prd-935c.bi_mart.stg_site_ref_1`;
      ...
      select
			*,
			...
			select 
				...
				region,
				gladiator_branch 
			from `data-bi-prd-935c.bi_mart.ref_site_h3i_mth` group by 1,2,3,4,5,6)a
		)a where rn=1
	)b
	on
	a.site_id=b.site_id
...
delete from `data-bi-prd-935c.bi_mart.site_ref_dim` where mth_sk_id = date_trunc(vdt_id,month);
...
select   
  ...
  remarks,
  CAST(ts AS DATETIME) 
from `data-bi-prd-935c.bi_mart.stg_site_ref_1`;