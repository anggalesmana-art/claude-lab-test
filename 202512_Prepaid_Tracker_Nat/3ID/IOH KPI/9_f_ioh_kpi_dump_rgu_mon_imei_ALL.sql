...
WITH tmp_vlr_band_mon AS (
    ...
    FROM (
        ...
        WHERE (CAST(load_dt AS DATE) >= DATE_TRUNC(vdt_id, MONTH) AND CAST(load_dt AS DATE) <= vdt_id)
    ) b
    WHERE rank = 1
),
    ...
    FROM (
        SELECT 
            ...
            ROW_NUMBER() OVER (PARTITION BY sbscrptn_ek_id ORDER BY 
                CASE WHEN band IS NULL THEN '0G' ELSE band END DESC, dt_sk_id DESC, SUBSTRING(served_imeisv, 1, 14)) AS rank
