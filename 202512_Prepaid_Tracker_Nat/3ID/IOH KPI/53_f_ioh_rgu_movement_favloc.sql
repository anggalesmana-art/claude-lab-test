...
WHERE CAST(ga_date AS DATE) IS NOT NULL
  AND CAST(ga_date AS DATE) >= DATE_TRUNC(vdt_id,MONTH)
  AND CAST(ga_date AS DATE) <= vdt_id 
GROUP BY 1, 2, 3, 4, 5;
...
where CAST(ga_date AS DATE) is not null
and CAST(ga_date AS DATE) >= DATE_TRUNC(date_sub(cast(vdt_id as date),interval 1 day), MONTH)
and CAST(ga_date AS DATE) <= date_sub(cast(vdt_id as date),interval 1 day)
group by 1,2,3,4,5;
...
LEFT JOIN (
    ...
    FROM (SELECT * 
            FROM `bi_mart.ioh_subscriber_site_attribs_rolling` 
      WHERE dt =cast(vdt_id as date)) a
    LEFT JOIN (
        SELECT site_id, gladiator_branch AS branch
        FROM (
            SELECT DISTINCT site_id, gladiator_branch, 
        ...
        WHERE a.rnk = 1
    ) b ON a.site_id_30 = b.site_id and a.dt=cast(vdt_id as date)
    --WHERE b.site_id IS NULL
) b ON cast(a.sbscrptn_ek_id as string) = b.sbscrptn_ek_id
WHERE a.tag = 'rgu30_churn_back'
  AND a.dt >= DATE_TRUNC(cast(vdt_id as date), MONTH)
  AND a.dt <= cast(vdt_id as date)
GROUP BY 1, 2, 3, 4, 5;
