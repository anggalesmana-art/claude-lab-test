...
insert into `data-bi-prd-935c.bi_mart.project_ioh_bima_daily_subs`
	...
	select    
		...
		where cast(load_dt_sk_id as date) = vdt_id and
...
WHERE dt_sk_id <= vdt_id
  AND dt_sk_id >= DATE_SUB(vdt_id, INTERVAL 30 DAY)
GROUP BY dt_sk_id, sbscrptn_msisdn;
...
-- and source_ctgry_2 in ('TRX_SUBSCRIBER_PURCHASE','TRX_POSTPAID_PAYMENT') and 
  ...
  group by 1
union all
select cast(dt_sk_id as date) as dt, 'monthly', count(distinct sbscrptn_msisdn)
from `data-bi-prd-935c.bi_mart.project_ioh_bima_monthly_subs`
where dt_sk_id = vdt_id
group by 1
union all
select cast(dt_sk_id as date) as dt, 'daily', count(distinct sbscrptn_msisdn)
from `data-bi-prd-935c.bi_mart.project_ioh_bima_daily_subs`
where dt_sk_id = vdt_id
group by 1
union all 
