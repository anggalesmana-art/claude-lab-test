...
---ret_qsso
		...
		select LEAST(CAST(first_usage_dt as date), PARSE_DATE('%Y%m%d',cast(first_billing_usage_dt_sk_id as string)),CAST(any_event_first_usage_date AS date))
		ga_date , fct.sbscrptn_ek_id
		from subs fct
