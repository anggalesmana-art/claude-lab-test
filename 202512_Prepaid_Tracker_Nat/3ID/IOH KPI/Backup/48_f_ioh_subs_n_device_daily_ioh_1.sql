...
least(cast(first_usage_dt as date), least(parse_date('%Y%m%d',cast(first_billing_usage_dt_sk_id as string)), cast(any_event_first_usage_date as date))) as first_usage_dt,
...
CAST(least(CAST(first_usage_dt AS DATE), least(parse_date('%Y%m%d',CAST(first_billing_usage_dt_sk_id AS STRING)), CAST(any_event_first_usage_date AS date))) AS DATE) as first_usage_dt,
'B2C' as Subs_Segment
...
and rgs_all_ex_sp and a.tool_of_trade_ind = 'N' and a.product_id = 8;
