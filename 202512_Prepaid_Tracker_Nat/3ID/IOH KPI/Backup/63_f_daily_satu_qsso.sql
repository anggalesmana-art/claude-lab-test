...
---ret_qsso
		...
		select LEAST(
				COALESCE(CAST(first_usage_dt AS DATE), vdt_id),
				COALESCE(PARSE_DATE('%Y%m%d', CAST(first_billing_usage_dt_sk_id AS STRING)), vdt_id),
				COALESCE(CAST(any_event_first_usage_date AS DATE), vdt_id)
			) AS ga_date
			, fct.sbscrptn_ek_id
		from subs fct
		...
		where LEAST(CAST(first_usage_dt as date), parse_date('%Y%m%d',cast(first_billing_usage_dt_sk_id as string)),CAST(any_event_first_usage_date as date)) 
