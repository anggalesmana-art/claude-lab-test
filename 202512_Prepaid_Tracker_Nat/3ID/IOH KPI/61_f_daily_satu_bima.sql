DECLARE vdt_id DATE DEFAULT @vdt_id;
		...
		delete from 
		`data-bi-prd-935c.bi_mart.project_ioh_kpi_daily_tracker_site` 
...
delete from `data-bi-prd-935c.bi_mart.project_ioh_kpi_daily_tracker_site` where load_dt_sk_id = vdt_id and kpi_code = 'M0SC_MTD';
...
delete from `data-bi-prd-935c.bi_mart.project_ioh_kpi_daily_tracker_site` where cast(load_dt_sk_id as date) = vdt_id 
...
delete from `data-bi-prd-935c.bi_mart.project_ioh_kpi_daily_tracker_site` where load_dt_sk_id = vdt_id and kpi_code = 'QSC';
...
where date_trunc(cast(created_dt_sk_id as date),month) = date_trunc(vdt_id,month)
),
base as 
(
	select * from `data-bi-prd-935c.bi_mart.fct_ga_site_id` b where date_trunc(b.dt,month) = date_trunc(vdt_id,month)
...
subs as (select * from `data-dtptechm-prd-c7ca.dwh.sbscrptn_attribs`)
...
delete from `data-bi-prd-935c.bi_mart.project_ioh_kpi_daily_tracker_site` where load_dt_sk_id = vdt_id 
...
base as 
(
select * from `data-bi-prd-935c.bi_mart.project_ioh_kpi_daily_tracker_site` b
...
WITH addres_site AS (
  ...
  WHERE mth = (
          SELECT MAX(mth) 
          FROM `data-bi-prd-935c.bi_mart.dm_phy_addressable_sites`
          WHERE mth <= DATE_TRUNC(DATE_SUB(vdt_id, INTERVAL 1 MONTH), MONTH)
        )
    AND UPPER(addressablesites) = 'ADDRESSABLE SITE'
),
...
base AS (
  SELECT *
  FROM `data-bi-prd-935c.bi_mart.project_ioh_kpi_daily_tracker_site` b
  WHERE b.load_dt_sk_id = vdt_id
...
FROM (
  SELECT 
    CAST(vdt_id AS DATE) ,
    'h3i',
    'Site_25QSC_250MB' AS kpi_code,
    'IOH',
    CAST(vdt_id AS DATE) ,
    site_id,
    COUNT(1) AS kpi_value,
    'site with 25 QSC 250' ,
  ...
  WHERE kpi_value >= 25 and x.site_id != 'N/A'
  GROUP BY 1, 2, 3, 4, 5, 6, 8, 9
);