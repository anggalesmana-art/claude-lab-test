...
`data-bi-prd-935c.bi_mart.fct_vlr_daily` 
...
`data-bi-prd-935c.bi_mart`.project_rguog_subs_detail a left  join subs  b
...
with subs as (select * from `data-dtptechm-prd-c7ca.dwh.sbscrptn_attribs`)
select cast(vdt_id as date) as load_Dt_sk_id,sbscrptn_msisdn From 
`data-bi-prd-935c.bi_mart`.project_rgumt_subs_detail a left join subs  b
on cast(a.sbscrptn_ek_id as string)=cast(b.sbscrptn_ek_id as string) 
where load_Dt_sk_id = vdt_id
group by 1,2 
) z 
...
left join 
subs b on a.sbscrptn_msisdn =b.sbscrptn_msisdn and b.rank_ind =1 
...
,count(distinct a.msisdn) 
from `data-bi-prd-935c.bi_mart.fct_vlr_daily` a left join 
subs b on a.msisdn =b.sbscrptn_msisdn and b.rank_ind =1  
