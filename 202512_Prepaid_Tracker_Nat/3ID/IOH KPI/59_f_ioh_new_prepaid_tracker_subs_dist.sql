...
group by  1,2,3,5,6,7; 
	...
	(
		select * from `data-bi-prd-935c.bi_mart.project_ioh_kpi_daily_tracker_site`b
		where  b.load_dt_sk_id =  vdt_id and b.kpi_code in ('scm0_any') and coalesce(value,0) >=25
	)
	select 
	'TRI' brand
	,''kpi_code
...
(
			...
			,	fl_number
			,	b.sbscrptn_ek_id, 
			--least(CAST(first_usage_dt as date), parse_date('%Y%m%d',cast(first_billing_usage_dt_sk_id as string)),CAST(any_event_first_usage_date as date)) 
			(
			SELECT MIN(d)
			FROM UNNEST([
