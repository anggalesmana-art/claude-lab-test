...
--- Temp 1
CREATE OR REPLACE TABLE `data-bi-prd-935c.bi_mart`.sms_addon2_{{ vdt_id }}_tmp1 as
SELECT msisdn, dt_id, subs_brand_sc_name, ifnull(a.ma_rev_code,a.da1_rev_code) rev_code, usg_svc_class_cd
, b.level_5, b.level_1, b.level_2, b.level_9, usg_hits, sms_rev, subs_tnr AS tnr
...
WHERE date(dt_id) = vdt_id
  AND SVC_USG_TP_REV = 'SMS'
  AND b.level_7 = 'SMS REG' ;
   
--- Insert
delete from `data-bi-prd-935c.bi_mart`.sms_addon_dly where dt_id = vdt_id;
INSERT into `data-bi-prd-935c.bi_mart`.sms_addon_dly 
SELECT msisdn, subs_brand_sc_name AS brnd_id, tnr, usg_svc_class_cd AS svc_class_code
, rev_code, level_1, level_2, usg_hits, sms_rev, pkg_original_price, pkg_sid, pkg_reg_channel
, pkg_pvrid, pkg_ngssp_transaction_id, pkg_commercial_name, timestamp(current_datetime('+7')) AS ppn_dttm, date(dt_id) dt_id 
FROM `data-bi-prd-935c.bi_mart`.sms_addon2_{{ vdt_id }}_tmp1 ;

--- Drop Temp Table
DROP TABLE `data-bi-prd-935c.bi_mart`.sms_addon2_{{ vdt_id }}_tmp1; 