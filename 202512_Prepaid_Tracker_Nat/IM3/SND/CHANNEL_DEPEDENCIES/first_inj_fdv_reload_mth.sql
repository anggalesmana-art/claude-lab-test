...
--/data/05/ioh/BI/schedule/mart/20231031_channel_depedencies/script/first_inj_fdv_reload_mth.sql

--- monthly temp
create or replace table `data-bi-prd-935c.bi_stg`.tmp_inj_fdv_reload_{{ vdt_id }} as
...
(
  select concat('62',pm_red_msisdn) msisdn, b.actvn_dt, mobo_inj_outlet_id organization_id
    ...
    , mobo_trx_id transaction_id, transactiondate dt_id
...
--- final temp
create or replace table `data-bi-prd-935c.bi_stg`.tmp_inj_fdv_reload_{{ vdt_id }}_fnl as
...
(
  ...
  (
    ...
    where mth_id = date(vdt_mth - interval 1 month)
      and actvn_dt>=vdt_mth - interval 24 month
      and inj_dt>=vdt_mth - interval 12 month
    union all
    select g.msisdn, date(g.actvn_dt) actvn_dt, g.organization_id, g.product_name, g.amount_debit, g.main_price, g.transaction_id, date(dt_id)
    from `data-bi-prd-935c.bi_stg`.tmp_inj_fdv_reload_{{ vdt_id }} g
    left join `data-bi-prd-935c.bi_mart`.first_inj_fdv_reload_mth h
...
delete from `data-bi-prd-935c.bi_mart`.first_inj_fdv_reload_mth  where mth_id = vdt_mth;
insert into `data-bi-prd-935c.bi_mart`.first_inj_fdv_reload_mth 
select a.*, timestamp(current_datetime('+7')) ppn_dttm, vdt_mth mth_id from `data-bi-prd-935c.bi_stg`.tmp_inj_fdv_reload_{{ vdt_id }}_fnl a
;

--- drop temp table
drop table if exists `data-bi-prd-935c.bi_stg`.tmp_inj_fdv_reload_{{ vdt_id }};
drop table if exists `data-bi-prd-935c.bi_stg`.tmp_inj_fdv_reload_{{ vdt_id }}_fnl;