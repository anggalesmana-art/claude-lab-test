...
--/data/05/ioh/BI/schedule/mart/20231031_channel_depedencies/script/first_inj_salmo_channel_mth.sql

-- monthly temp
create or replace table `data-bi-prd-935c.bi_stg`.tmp_inj_salmo_channel_{{ vdt_id }} as
...
--- final temp
delete from `data-bi-prd-935c.bi_mart`.first_inj_salmo_channel_mth  where mth_id = vdt_mth;
insert into `data-bi-prd-935c.bi_mart`.first_inj_salmo_channel_mth 
select msisdn, actvn_dt, channel, organization_id, product_name, amount_debit, main_price, transaction_type, transaction_id, inj_dt
  , timestamp(current_datetime('+7')) ppn_dttm, vdt_mth mth_id
from
(
  select a.*, row_number() over(partition by msisdn
    , case
      when lower(channel) in ('direct') then 'DSF'
      when lower(channel) in ('traditional') then 'Traditional'
      else 'Modern' end
    order by actvn_dt desc, inj_dt, amount_debit desc) rk
  from
  (
    select msisdn, actvn_dt, channel, organization_id, product_name, amount_debit, main_price, transaction_type, transaction_id, inj_dt
    from `data-bi-prd-935c.bi_mart`.first_inj_salmo_channel_mth
    where mth_id=vdt_mth - interval 1 month
      and actvn_dt>=vdt_mth - interval 24 month
      and inj_dt>=vdt_mth - interval 12 month
    union all
    select g.msisdn, date(g.actvn_dt), g.channel, g.organization_id, g.product_name, g.amount_debit, g.main_price, g.transaction_type, g.transaction_id, date(g.dt_id)
    from `data-bi-prd-935c.bi_stg`.tmp_inj_salmo_channel_{{ vdt_id }} g
    left join `data-bi-prd-935c.bi_mart`.first_inj_salmo_channel_mth h
      on g.msisdn=h.msisdn and date(g.actvn_dt)=h.actvn_dt and h.mth_id=vdt_mth - interval 1 month
    and g.channel=h.channel
...
--- drop temp table
drop table if exists `data-bi-prd-935c.bi_stg`.tmp_inj_salmo_channel_{{ vdt_id }};