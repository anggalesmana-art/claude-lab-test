...
--- union all
delete from `data-bi-prd-935c.bi_mart`.first_inj_channel_mth where mth_id = vdt_mth;
insert into `data-bi-prd-935c.bi_mart`.first_inj_channel_mth 
...
(
  ...
  (
    ...
    select msisdn, actvn_dt
      --, case when regexp_like(organization_id,"^[0-9]+$")=False then 'DSF' else 'Traditional' end channel /* updated at 20230112 */
      , channel
