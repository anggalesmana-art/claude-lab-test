...
delete from `data-bi-prd-935c.bi_mart`.sms_lacci_dly where dt_id = vdt_id;
insert into `data-bi-prd-935c.bi_mart`.sms_lacci_dly 
...
from (
  ...
  where service_type IN ('SMSMO')
    --AND dt_id = '${hiveconf:CURRENT_DATE}'
  ...
  --length(called_party_number) >= 10 and length(called_party_number) < 17 and
    and substr(calling_party_number, 1, 5) IN ('62814', '62815', '62816', '62855', '62856', '62857', '62858')
  union ALL
  -- incoming
  ...
  from `data-dtp-prd-aa1a.sor.msc_detail`
  where service_type IN ('SMSMT')
    AND date(dt_id) = vdt_id
  --length(calling_party_number) >= 10 and length(calling_party_number) < 17 and
  --length(called_party_number) >= 10 and length(called_party_number) < 17 and
    and substr(called_party_number,1,5) IN ('62814', '62815', '62816', '62855', '62856', '62857', '62858')
) a
