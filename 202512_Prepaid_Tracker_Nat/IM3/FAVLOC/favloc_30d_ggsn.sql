...
CREATE OR REPLACE TABLE
  `data-bi-prd-935c.bi_stg`.tmp_ggsn_30d_agg_{{ vdt_id }}
...
CREATE OR REPLACE TABLE
  `data-bi-prd-935c.bi_stg`.tmp_favloc_30d_ggsn_{{ vdt_id }} AS
SELECT
  msisdn,
...
FROM (
  ...
  FROM `data-bi-prd-935c.bi_stg`.tmp_ggsn_30d_agg_{{ vdt_id }}
)
WHERE idx = 1;

DROP TABLE IF EXISTS `data-bi-prd-935c.bi_stg`.tmp_ggsn_30d_agg_{{ vdt_id }};
