...
-- INSERT OVERWRITE TABLE biadm.rk_cst_pstpaid_churn_detail_v3 PARTITION(dt_id)
delete from `data-bi-prd-935c.bi_dev.cst_pstpaid_churn_detail_v3`
where dt_id = observation_date;

insert into `data-bi-prd-935c.bi_dev.cst_pstpaid_churn_detail_v3`
...
CHURNPRE AS (
    select 
        ...
        a.dt_id
    from (
        select * from `data-bi-prd-935c.bi_mart.cst_pstpaid_churn_v1`
        where dt_id = observation_date
    ) a
