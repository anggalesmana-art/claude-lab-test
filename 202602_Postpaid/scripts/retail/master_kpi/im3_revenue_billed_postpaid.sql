...
with payment_data as (
	...
	where a.order_type in ('New Registration','Migration', 'Port In Migration')
		and (lower(a.channel_rev) like '%gerai%' or lower(a.channel_rev) like '%franchise%')
		and left(a.pymt_dt,6) = left('{dt_id}',6)
		and a.pymt_dt <= '{dt_id}'
		and left(a.activation_date,6)>=FORMAT_DATE('%Y%m', DATE_SUB(PARSE_DATE('%Y%m%d', '{dt_id}'),INTERVAL 3 MONTH))
)
select
	'{dt_id}' as dt_id,
	'rev_billed_postpaid' as kpi_name,
	'agent' as level, 
	nik_sales as agent_id, 
	store_code_rev as store_code, 
	'IM3' as brand, 
	sum(pymt_amt) as value
from payment_data as a
