	...
	from (
		 	...
		 	* 
		 	from(
				...
				from `data-bi-prd-935c.bi_mart.cst_pstpaid_ga_v1`
					where 
						date(dt_id)=(select max_dt from max_ga_bi)
						-- date(dt_id)=PARSE_DATE('%Y%m%d', '{dt_id}')
						-- date(dt_id)=DATE_SUB(PARSE_DATE('%Y%m%d', '{dt_id}'), INTERVAL 1 DAY)
		...
		left join (
			...
			from `data-bi-prd-935c.bi_mart.cst_pstpaid_ga_v1`
				where date(dt_id)between date(SUBSTR('{dt_id}',1,4) || '-' || SUBSTR('{dt_id}',5,2) || '-01') 
					-- and PARSE_DATE('%Y%m%d', '{dt_id}')
					and (select max_dt from max_ga_bi)
					and flag like '1%'
			) as y where y.rnk=1
		) as b
...
select * from (
	select 
				...
				format_date('%Y%m%d', DATETIME(a.completion_date,'Asia/Jakarta')) as activation_date,
				a.account_num, 
				a.msisdn,
...
--	left join  `data-nationalslsdist-prd-986g`.postpaid.tmp_daily_ga as c
...
, ga_verf_new as(
	...
	SELECT DISTINCT 
		format_date('%Y%m%d',SAFE.PARSE_DATE('%m/%d/%Y', LEFT(a.activation_date, 10))) AS activation_date,
		REPLACE(a.account_num, '\\', '') AS account_num,
			REPLACE(a.msisdn, '\\', '') AS msisdn,
			CONCAT(a.cust_first_name, cust_last_name) AS cust_name,
			SAFE_CAST(a.package_name AS STRING) AS package_name,
			a.name AS order_type,
			LEFT(a.galeri_name, 4) AS dealerid,
			SUBSTR(a.galeri_name, 6, 50) AS store,
			REPLACE(a.id_number, '\\', '') AS id_number,
			a.salesperson_nm,
			`comment` AS comment_text,
			a.order_created_by AS created,
			a.outlet_code,
			a.outlet_name,
			a.galeri_name,
			a.branch_name,
			a.region_name,
			LEFT(a.galeri_name, 4) AS store_code,
			IF(ARRAY_LENGTH(SPLIT(order_created_by, '-')) > 0, SPLIT(order_created_by, '-')[OFFSET(ARRAY_LENGTH(SPLIT(order_created_by, '-')) - 1)], NULL) AS sales_name,
			CASE 
			WHEN ARRAY_LENGTH(SPLIT(order_created_by, '-')) = 2 THEN SPLIT(order_created_by, '-')[OFFSET(0)]
			WHEN ARRAY_LENGTH(SPLIT(order_created_by, '-')) = 3 THEN CONCAT(SPLIT(order_created_by, '-')[OFFSET(0)], '-', SPLIT(order_created_by, '-')[OFFSET(1)])
			WHEN ARRAY_LENGTH(SPLIT(order_created_by, '-')) = 4 THEN CONCAT(SPLIT(order_created_by, '-')[OFFSET(0)], '-', SPLIT(order_created_by, '-')[OFFSET(1)], '-', SPLIT(order_created_by, '-')[OFFSET(2)])
			END AS nik_sales,
			row_number() over(partition by REPLACE(a.msisdn, '\\', '') order by format_date('%Y%m%d',SAFE.PARSE_DATE('%m/%d/%Y', LEFT(a.activation_date, 10))) desc) as rnk
	FROM `data-nationalslsdist-prd-986g`.postpaid.raw_new_verification as a
		left join ga_new as b
			on a.msisdn=b.msisdn
		WHERE SAFE.PARSE_DATE('%m/%d/%Y', SUBSTR(a.activation_date, 1, 10)) <= PARSE_DATE('%Y%m%d', '{dt_id}')
			AND FORMAT_DATE('%Y%m', SAFE.PARSE_DATE('%m/%d/%Y', SUBSTR(a.activation_date, 1, 10))) = SUBSTR('{dt_id}', 1, 6)
			and b.msisdn is null
...
, daily_ga as (
	...
	left join (
		...
		where update_dt = (select max(update_dt) from `data-nationalslsdist-prd-986g`.postpaid.ref_storelist)
			and concat(store_code, circle)<>'SM18JAVA'
	) as b 
	...
	left join (
		select * from (
			...
			from `data-nationalslsdist-prd-986g`.postpaid.ref_storelist  
			where concat(store_code, circle)<>'SM18JAVA'
			-- where update_dt = (select max(update_dt) from `data-nationalslsdist-prd-986g`.postpaid.ref_storelist)
		) as x where x.rnk=1
	) as d
...
select
*, row_number() over(partition by msisdn order by activation_date) as idx
from final_ga 
) as x where x.idx=1
; 
...
from (
	...
	where date(b.activation_date) between date(SUBSTR('{dt_id}',1,4) || '-' || SUBSTR('{dt_id}',5,2) || '-01') 
						and PARSE_DATE('%Y%m%d', '{dt_id}')
						and a.dealer_id not in ('MYM3-CVM' , 'OLA1')
) as b
...
where a.msisdn=b.msisdn and a.activation_date=b.activation_date
	and dealer_id not in ('MYM3-CVM' , 'OLA1')
;
...
-- xx. update storelist ps
update `data-nationalslsdist-prd-986g`.postpaid.raw_daily_ga as a 
	set 
	store_code_rev=b.store_code, 
	store_name_rev=b.store_name,
	sales_area=b.branch, 
	region_new_2024=b.region,
	circle=b.circle,
	...
	bsm=b.branch,
	channel_partner_store_detail=b.channel_detail,
	promotor=b.promotor_name,
	channel_rev = 'PARTNER STORE'
	from (
	select 
		a.msisdn,
		a.store_code_rev,
		...
		b.rce, 
		a.activation_date,
		b.activation_date as queueing_date,
		b.product_name,
		b.channel_group,
		b.channel_detail,
		b.branch,
		...
		(
			...
			select * from (
					...
					from `data-nationalslsdist-prd-986g`.postpaid.raw_ipos_ps_trx
						where (lower(product_name) like '%platinum%' or lower(product_name) like '%prime%')
						-- and PARSE_TIMESTAMP('%d/%m/%Y %H:%M:%S', approved_date)>=TIMESTAMP_SUB(TIMESTAMP(PARSE_DATE('%Y%m%d', '${dt_id}')), INTERVAL 45 DAY)
						and (PARSE_TIMESTAMP('%d/%m/%Y %H:%M:%S', approved_date)>=TIMESTAMP_SUB(TIMESTAMP(PARSE_DATE('%Y%m%d', '${dt_id}')), INTERVAL 60 DAY)
						or PARSE_TIMESTAMP('%d/%m/%Y %H:%M:%S', transaction_date)>=TIMESTAMP_SUB(TIMESTAMP(PARSE_DATE('%Y%m%d', '${dt_id}')), INTERVAL 60 DAY)
						)
						and status='Success'
				) as x where x.idx=1
			) as a 	
			...
			left join (
				select * from (
					...
					*,
					...
					) as c on a.store_code=c.store_code
		)
		as b
			on a.msisdn=b.msisdn 
				-- and a.activation_date=b.activation_date 
				-- and a.store_code_rev<>b.store_code 
		where left(a.activation_date,6)=left('${dt_id}',6)
			and a.dealer_id='DC11'
			and (a.store_code_rev is null or a.store_code_rev='Others' or a.store_code_rev='')
) as b
	where a.msisdn=b.msisdn
		and a.activation_date=b.activation_date
		and a.dealer_id='DC11'
