...
select 
	...
	a.sales_name,
	a.nik_sales,
	case when a.dealer_id in ('ALTC','DC11','DCCS','JKBB','JKBA','OLA1') and (d.store_code is not null or e.store_code is not null or f.store_code is not null) then coalesce(d.store_code,e.store_code,f.store_code) 
	...
	left join (
			select * from (
				select 
					...
					invoice_number,
					row_number() over(partition by customer_msisdn order by PARSE_TIMESTAMP('%d/%m/%Y %H:%M:%S', approved_date) desc, 
...
, daily_ga as (
	select
	a.*, 
	coalesce(store.region,b.region_circle) as region_rev, 
	coalesce(branch.area,b.area) as area_rev, 
	coalesce(branch.area,b.sales_area) as sales_area_rev, 
	coalesce(store.channel_group,b.channel_group) as channel_rev, 
	coalesce(store.store_name,b.store_name) as store_name_rev, 
	coalesce(store.channel_group,b.channel_group) as channel_partner_store_group, 
	...
	left join (select * from `data-nationalslsdist-prd-986g`.postpaid.ref_promotor_mapping 
	...
	left join (select * from `data-nationalslsdist-prd-986g`.postpaid.ref_dsa_mapping 
	...
	left join (
		...
		on case when a.channel='Direct Sales' then upper(c.store_code) else upper(a.store_code_rev) end=upper(store.store_code) 
	left join (
		select 
			* 
...
select 
	...
	a.sales_name,
	a.nik_sales,
	case when a.salesperson_nm in (select distinct store_code  from `data-nationalslsdist-prd-986g`.postpaid.ref_storelist_base rsb where lower(channel_group) like 'partner store%') 
		...
		when a.dealer_id in ('ALTC','DC11','DCCS','JKBB','JKBA','OLA1') 
				...
				and d.store_code<>'IMO01' then coalesce(d.store_code,e.store_code,f.store_code) 
		when a.dealer_id not in ('ALTC','DC11','DCCS','JKBB','JKBA','OLA1', 'SMB0') then a.dealer_id 
		when a.dealer_id ='DCCS' and d.store_code is null and e.store_code is null and f.store_code is null then 'MYIM3OLA' 
		when a.dealer_id='SMB0' and a.galeri_name like 'SMB%' then left(a.galeri_name,6)
		when a.dealer_id is null or a.dealer_id='' or a.salesperson_nm='MYM3-CVM' then 'MYIM3OLA'
			...
			select * from (
				select 
					...
					row_number() over(partition by customer_msisdn order by case when status='Success' then 1 else 2 end asc, PARSE_TIMESTAMP('%d/%m/%Y %H:%M:%S', approved_date) desc, 
							PARSE_TIMESTAMP('%d/%m/%Y %H:%M:%S',transaction_date) desc) as idx
					-- before change only check order by approved date
					-- row_number() over(partition by customer_msisdn order by PARSE_TIMESTAMP('%d/%m/%Y %H:%M:%S', approved_date) desc, 
					-- 		PARSE_TIMESTAMP('%d/%m/%Y %H:%M:%S',transaction_date) desc) as idx
				from `data-nationalslsdist-prd-986g`.postpaid.raw_ipos_ps_trx
					where (lower(product_name) like '%platinum%' or lower(product_name) like '%prime%')
...
, daily_ga as (
	...
	coalesce(store.channel_group,b.channel_group) as channel_partner_store_group, 
	coalesce(store.channel_detail,b.channel_detail) as channel_partner_store_detail, 
	case when coalesce(c.store_code, a.nik_sales) not in ('Administrator', 'EAIUSER') then coalesce(c.store_code, a.nik_sales) end as nik_sales_rev, 
	e.full_name as sales_name_rev, 
	cast(null as string) as no_imkas_sales,
	cast(null as string) as npwp_sales,
	cast(null as string) as alamat_sales,
