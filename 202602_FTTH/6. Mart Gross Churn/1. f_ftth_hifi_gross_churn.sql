CREATE OR REPLACE PROCEDURE `data-bi-prd-935c.bi_dm.f_ftth_hifi_gross_churn`(IN starts_date DATE, IN observation_date DATE)
BEGIN
  ...
  WHILE starts_date <= observation_date DO
    ...
    delete from `data-bi-prd-935c.bi_dm.ftth_cst_gross_churn_v1`
    where dt_id = starts_date
    and flag_customer = "HIFI";

    INSERT INTO `data-bi-prd-935c.bi_dm.ftth_cst_gross_churn_v1`
    WITH PREPARATIONHIFI AS (
      select
        ...
        site_id,
        dt_id
      from `data-bi-prd-935c.bi_dm.tysg_hifi_hist_gc_v1` 
      where dt_id = starts_date    
    ),
    GROSSCHURNMTD AS (
      select
        '2. GROSS CHURN (EOP)' flag,
        a.flag_customer,
    ...
    GROSSCHURNDAILY AS (
      ...
      on a.billing_account = b.billing_account
      left join (
        select * from `data-bi-prd-935c.bi_dm.tysg_hifi_hist_gc_v1`
        where dt_id = starts_date
      ) c
      on a.billing_account = c.billing_account
      where b.billing_account is null
    ),
    GROSSCHURNMTDFINAL AS ( 
      select
          distinct
          a.flag,
          a.flag_customer,
          ...
          coalesce(b.gross_churn_date,starts_date) gross_churn_date,
          ...
          starts_date dt_id
      from GROSSCHURNMTD a 
      left join (
        select * from `data-bi-prd-935c.bi_dm.ftth_cst_gross_churn_v1`
        where dt_id = yday_date
        and flag like '%2%'
        and flag_customer = 'HIFI'
      ) b
    ...
    select * from GROSSCHURNMTDFINAL
    union all
    select * from GROSSCHURNDAILY;
