CREATE OR REPLACE PROCEDURE `data-bi-prd-935c.bi_dm.f_ftth_mnc_gross_churn`(IN starts_date DATE, IN observation_date DATE)
BEGIN
  ...
  WHILE starts_date <= observation_date DO
    ...
    WITH PREPARATIONMNC AS (
      select
        ...
        site_id,
        dt_id
      from `data-bi-prd-935c.bi_dm.tysg_mnc_hist_gc_v1` 
      where dt_id = starts_date    
    ),
    GROSSCHURNMTD AS (
      select
