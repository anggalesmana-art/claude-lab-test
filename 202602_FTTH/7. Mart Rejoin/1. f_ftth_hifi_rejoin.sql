CREATE OR REPLACE PROCEDURE `data-bi-prd-935c.bi_dm.f_ftth_hifi_rejoin`(IN starts_date DATE, IN observation_date DATE)
BEGIN
  ...
  WHILE starts_date <= observation_date DO
    ...
    WITH PREPARATIONHIFI AS (
      select
        ...
        site_id,
        dt_id
      from `data-bi-prd-935c.bi_dm.tysg_hifi_hist_rejoin_v1` 
      where dt_id = starts_date    
    ),
    REJOINMTD AS (
      select
    ...
    REJOINDAILY AS (
      ...
      on a.billing_account = b.billing_account
      left join (
        select * from `data-bi-prd-935c.bi_dm.tysg_hifi_hist_rejoin_v1`
        where dt_id = starts_date
      ) c
